...
export default function Footer({ onNavigate }: { onNavigate: (p: Page) => void }) {
  ...
  return (
    <footer style={{ background: '#0F172A', color: 'white' }}>
      ...
      ── */}
      <div style={{ background: `linear-gradient(135deg, ${branding.primaryColor} 0%, #1E3A8A 100%)`, padding: '56px 24px' }}>
        ...
        <div className="max-w-[1200px] mx-auto" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 40 }}>
          ...
          <Stack gap={16}>
            <Row gap={10} wrap={false}>
              <div style={{ width: 36, height: 36, borderRadius: 10, background: branding.primaryColor, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', flexShrink: 0 }}>
                <div style={{ width: 20, height: 20 }}>{Icon.shield}</div>
              </div>
              <span style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 18, letterSpacing: '-0.02em', whiteSpace: 'nowrap' }}>{branding.companyName}</span>
            </Row>
          ...
          <Stack gap={8}>
            ...
            {[
              { icon: Icon.phone, text: branding.supportPhone },
              { icon: Icon.mail, text: branding.supportEmail },
              { icon: Icon.pin, text: branding.address },
              { icon: Icon.whatsapp, text: `WhatsApp: ${branding.whatsapp}` },
            ].map(c => (
              /* Each contact row is horizontal auto layout */
              <Row key={c.text} gap={10} wrap={false} align="flex-start">
                <div style={{ width: 14, height: 14, color: branding.primaryColor, flexShrink: 0, marginTop: 2 }}>{c.icon}</div>
